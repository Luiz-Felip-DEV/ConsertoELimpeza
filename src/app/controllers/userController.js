...
class userController {
  ...
  async setLoginAction(req, res)
  {
    ...
    try {
      ...
      return res.status(500).json({
        error: true,
        msgUser:
          "Desculpe, ocorreu um erro ao tentar fazer o login. Verifique se todos os campos foram preenchidos corretamente e tente novamente. Se o problema persistir, entre em contato conosco para assistência.",
    ...
    if (verify) {
      return res.status(400).json({
        error: true,
        msgUser:
          "O acesso foi negado devido a informações incorretas. Verifique suas credenciais e faça login novamente.",
    ...
    return res.status(200).json({
      error: false,
      msgUser:
        "Acesso concedido! Você entrou com sucesso. Explore todas as funcionalidades disponíveis.",
      msgOriginal: null,
      jwt: jwt,
      results: arrDados,
    });
  ...
  async setUserAction(req, res) 
  {
    ...
    if (await userUtils.verifyCpf(req.body.cpf)) {
      return res.status(400).json({
        error: true,
        msgUser:
          "O CPF fornecido já está associado a uma conta existente. Por favor, use um CPF diferente para se cadastrar.",
        msgOriginal: null,
      });
    }
    ...
    if (await userUtils.verifyTelephone(req.body.telefone)) {
      return res.status(400).json({
        error: true,
        msgUser:
          "O Telefone fornecido já está associado a uma conta existente. Por favor, use um Telefone diferente para se cadastrar.",
        msgOriginal: null,
      });
    }
    ...
    if (await userUtils.verifyEmail(req.body.email)) {
      return res.status(400).json({
        error: true,
        msgUser:
          "O email fornecido já está associado a uma conta existente. Por favor, use um Email diferente para se cadastrar.",
        msgOriginal: null,
      });
    }
    ...
    try {
      ...
      return res.status(500).json({
        error: true,
        msgUser:
          "Desculpe, ocorreu um erro ao tentar cadastrar usuario. Verifique se todos os campos foram preenchidos corretamente e tente novamente. Se o problema persistir, entre em contato conosco para assistência.",
  ...
  async updateUser(req, res) {
    ...
    try {
      ...
      await userRepository.insertLog(
        await userUtils.insertLog(e.message, "Error", "Update", idUser)
      );

      return res.status(500).json({
        error: true,
        msgUser:
          "Desculpe, ocorreu um erro ao tentar atualizar dados do usuario. Verifique se todos os campos foram preenchidos corretamente e tente novamente. Se o problema persistir, entre em contato conosco para assistência.",
    ...
    if (verify) {
      await userRepository.insertLog(
        await userUtils.insertLog(
          "Retorno da base de dados vazia.",
          "Error",
          "Update",
          idUser
        )
  ...
  async deleteUser(req, res) {
    ...
    try {
      ...
      return res.status(500).json({
        error: true,
        msgUser:
          "Desculpe, ocorreu um erro ao tentar excluir dados do usuario. Se o problema persistir, entre em contato conosco para assistência.",
  ...
  async getDadosUser(req, res) {
    ...
    try {
      ...
      return res.status(500).json({
        error: true,
        msgUser:
          "Desculpe, ocorreu um erro ao tentar procurar dados do usuario. Se o problema persistir, entre em contato conosco para assistência.",
    ...
    if (verify) {
      return res.status(400).json({
        error: true,
        msgUser:
          "Nenhum usuario encontrado. Se o problema persistir, entre em contato conosco para assistência.",
        msgOriginal: "Não encontrou o usuario no banco.",
      });
    }
  ...
  async resetCode(req, res) {
    ...
    try {
      ...
      return res.status(500).json({
        error: true,
        msgUser:
          "Ocorreu um erro interno ao processar sua solicitação de redefinição de senha. Por favor, tente novamente mais tarde.",
    ...
    if (verify) {
      return res.status(400).json({
        error: true,
        msgUser:
          "O e-mail fornecido não está associado a uma conta. Verifique se você digitou corretamente o e-mail ou registre-se para criar uma nova conta.",
    ...
    return res.status(200).json({
      error: false,
      msgUser:
        "Um código de redefinição de senha foi enviado para o seu endereço de e-mail.",
      msgOriginal: null,
      codigo: code,
    });
  }
